...
class DeviceConnector(QThread):
    ...
    def run(self):
        success = test_connection.auto_connect_headset(self.mac)
        self.result_signal.emit(self.mac, success)
...
class BTManager(QWidget):
    ...
    def connect_to_bt_device(self, item):
        ...
        original_name = label.text().replace(" (connected)", "").replace(" (Connecting...)", "")
        label.setText(f"{original_name} (Connecting...)")
        ...
        self.device_connector = DeviceConnector(new_mac_addr)
        self.device_connector.result_signal.connect(lambda mac, success, item=item, original_name=original_name: self.handle_connection_result(mac, success, item, original_name))
        self.device_connector.start()

    def handle_connection_result(self, mac, success, item, original_name):
        old_data = item.data(Qt.UserRole)
        clean_name = original_name.replace(" (connected)", "").replace(" (Connecting...)", "")
        ...
        if success:
            ...
            def show_connected():
                ...
                device_name = clean_name + " (connected)"

                new_item = QListWidgetItem()
                new_item.setSizeHint(QSize(250, 70))
                self.mac_to_device[id(new_item)] = mac
                ...
                new_button.clicked.connect(lambda _, m=mac, i=new_item: self.disconnect_bt_device(m, i))
            ...
            QTimer.singleShot(400, show_connected)
        else:
