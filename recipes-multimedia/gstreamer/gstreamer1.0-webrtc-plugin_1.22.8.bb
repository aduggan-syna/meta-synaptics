...
do_install:append() {
    # Create the GStreamer plugin directory
    install -d ${D}${libdir}/gstreamer-1.0
    # Copy the compiled GStreamer plugin .so files to the GStreamer directory
    for lib in ${D}${libdir}/rustlib/aarch64-poky-linux-gnu/lib/*.so; do
        install -m 0755 ${lib} ${D}${libdir}/gstreamer-1.0/
    done
    # Remove the rustlib directory to ensure libraries only appear in gstreamer-1.0
    rm -rf ${D}${libdir}/rustlib
