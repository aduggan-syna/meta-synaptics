...
if [ "$MACHINE" = "sl2611" ]; then
    cp $OUTPUT_DIR/SYNAIMG/boot.subimg.gz $OUTPUT_DIR/SYNAIMG/bl.subimg.gz $OUTPUT_DIR/SYNAIMG/sysmgr.subimg.gz $OUTPUT_DIR/SYNAIMG/tzk.subimg.gz $OUTPUT_DIR/SYNAIMG/key.subimg.gz $OUTPUT_DIR/SYNAIMG/preboot.subimg.gz $RESCUE_OTA/

    gzip -df $RESCUE_OTA/boot.subimg.gz $RESCUE_OTA/bl.subimg.gz $RESCUE_OTA/sysmgr.subimg.gz $RESCUE_OTA/tzk.subimg.gz $RESCUE_OTA/key.subimg.gz $RESCUE_OTA/preboot.subimg.gz
else
    cp $OUTPUT_DIR/SYNAIMG/fastlogo.subimg.gz $OUTPUT_DIR/SYNAIMG/boot.subimg.gz $OUTPUT_DIR/SYNAIMG/bl.subimg.gz $OUTPUT_DIR/SYNAIMG/sysmgr.subimg.gz $OUTPUT_DIR/SYNAIMG/tzk.subimg.gz $OUTPUT_DIR/SYNAIMG/key.subimg.gz $OUTPUT_DIR/SYNAIMG/preboot.subimg.gz $RESCUE_OTA/

    gzip -df $RESCUE_OTA/fastlogo.subimg.gz $RESCUE_OTA/boot.subimg.gz $RESCUE_OTA/bl.subimg.gz $RESCUE_OTA/sysmgr.subimg.gz $RESCUE_OTA/tzk.subimg.gz $RESCUE_OTA/key.subimg.gz $RESCUE_OTA/preboot.subimg.gz
fi
...
HASH_SYSMGR=$(sha256sum "$RESCUE_OTA/$SYSMGR" | awk '{ print $1 }')
...
(
    ...
    if [ $2 -eq 1 ]; then
        ...
        (echo sw-description; echo sw-description.sig; find . -name "$PREBOOT" -o -name "$KEY" -o -name "$TZK" -o -name "$SYSMGR" -o -name "$BL" -o -name "$BOOT" -o -name "$ROOTFS" -o -name "$FASTLOGO") | cpio -o --format=newc > $SWU_IMAGE
    elif [ $2 -eq 2 ]; then
        if [ ! -f public.pem ]; then
            echo "test" > passout
            openssl genrsa -aes256 -passout file:passout -out priv.pem
        ...
        #CPIO ARCHIVE (Remove or include the images in the below command as per the requirement)
        (echo sw-description; find . -name "$PREBOOT" -o -name "$KEY" -o -name "$TZK" -o -name "$SYSMGR" -o -name "$BL" -o -name "$BOOT" -o  -name "$ROOTFS" -o -name "$FASTLOGO") | cpio -o --format=newc > $SWU_IMAGE
    fi
    if [ $? -eq 0 ]; then
